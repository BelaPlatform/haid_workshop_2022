...
unsigned int gMaxHapticOscFreqSliderId;	// Max Frequency of haptic oscillator
unsigned int gMinHapticOscFreqSliderId;	// Min Frequency of haptic oscillator
...
bool setup(BelaContext *context, void *userData)
{
	...
	gMinHapticOscFreqSliderId = controller.addSlider("[min] Haptic OSC F",(int)(0.5*(gHapticOscFreqRange[1] - gHapticOscFreqRange[0])) , gHapticOscFreqRange[0], gHapticOscFreqRange[1], 1);	// Frequency of haptic oscillator
	gMaxHapticOscFreqSliderId = controller.addSlider("[max]Haptic OSC F",gHapticOscFreqRange[1] , gHapticOscFreqRange[0], gHapticOscFreqRange[1], 1);	// Frequency of haptic oscillator
...
void render(BelaContext *context, void *userData)
{
	...
	float maxHapticOscFreq = controller.getSliderValue(gMaxHapticOscFreqSliderId);		// Max Frequency of haptic oscillator
	float minHapticOscFreq = controller.getSliderValue(gMinHapticOscFreqSliderId);		// Min Frequency of haptic oscillator
	...
	for(unsigned int n = 0; n < context->audioFrames; n++) {
		...
		if(gNumActiveTouches > 0)
		{
			...
			float hapticOscFreq = map(gTouchLocation[0], 0, 1, minHapticOscFreq, maxHapticOscFreq);
