...
void render(BelaContext *context, void *userData)
{
	...
	for(unsigned int n = 0; n < context->audioFrames; n++) {
		...
		if(gNumActiveTouches != gPrevNumActiveTouches)
		{
			gOscOnCount = (int)(context->audioSampleRate * oscOnTimeMs * 0.001); // Reset counter
			
			// Scale haptic oscillator frequency based on number of touches
			float hapticOscFreq = map(gNumActiveTouches, 0, NUM_TOUCH, hapticMinOscFreq, gHapticOscFreqRange[1]);
